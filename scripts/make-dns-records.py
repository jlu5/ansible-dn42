...
def write_forward_zone(domain, records):
    ...
    if domain == global_vars['dns_domain']:
        for router in public_hosts:
            assert global_vars['dns_auto_host_record_suffix'].startswith('.')
            router_hostname = router + global_vars['dns_auto_host_record_suffix']
...
def _load_config():
    global hosts, public_hosts
    yaml_data = yaml_load(args.hosts)
    hosts = get_hosts(yaml_data, include_private=True)
    public_hosts = get_hosts(yaml_data)
